...
### Verification Queries — Mapping to Research Questions

| #   | UPPAAL Formula                                            | RQ          | What it proves                                                                                                                                         |
| --- | --------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Q1  | `E<> not deadlock`                                        | **RQ1+RQ3** | A valid execution path exists — the automated transformation produces a viable model, not a broken one                                                 |
| Q2  | `E<> executor_X.local_clock > 0 and <= 100000`            | **RQ1**     | Timing constraints from LTTng trace are reachable in the model — proves timing semantics survive the 4-stage pipeline                                  |
| Q3  | `E<> message_count > 0`                                   | **RQ2+RQ3** | Message delivery is reachable — QoS RELIABLE delivery guarantee is representable in the generated UPPAAL model                                         |
| Q4  | `E<> system_running and global_clock > 0`                 | **RQ1**     | System reaches an operational state with elapsed time — ROS 2 operational timing semantics are preserved post-transformation                           |
| Q5  | `E<> executor_X.callback_count > 0` (subscriber executor) | **RQ1+RQ3** | Subscriber executor reachably executes — trace-extracted CBG_Executor behaviour is reproducible in generated model, confirming trace-to-model fidelity |
| Q6  | `E<> executor_X.callback_count > 0` (timer executor)      | **RQ3**     | Timer callback reachably fires — CBG_Executor periodic scheduling semantics captured by automated transformation                                       |
| Q7  | `E<> message_count > 0 and <= MAX_RELEASES`               | **RQ2**     | QoS throughput bound reachable and respected — model encodes both delivery and QoS history constraints from trace                                      |
| Q8  | `E<> is_executing and callback_count > 0`                 | **RQ3**     | CBG_Executor execution consistency reachable — timer executor reaches active state with registered callbacks, matching trace semantics                 |
